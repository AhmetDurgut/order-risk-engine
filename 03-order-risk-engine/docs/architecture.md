...
|---|---|---|---|
| `CREDIT_LIMIT` | `UKMBP_CMS_SGM` credit limit (FSCM) + `BSID` open items | 25 | Usage = (open balance + this order) / credit limit. ≥100% → full score; ≥90% → 70%; ≥80% → 40%; below that → 0. |
...
|---|---|---|---|
| `CREDIT_LIMIT` | `UKMBP_CMS_SGM` kredi limiti (FSCM) + `BSID` açık kalemler | 25 | Kullanım = (açık bakiye + bu sipariş) / kredi limiti. ≥%100 → tam skor; ≥%90 → %70; ≥%80 → %40; altında → 0. |
