...
| ZCL_ORDER_RISK_CHECKER | `check_credit_limit` | `SELECT` UKMBP_CMS_SGM (FSCM kredi limiti) ve BSID (açık bakiye) |
...
| ZCL_ORDER_RISK_CHECKER | `check_credit_limit` | `SELECT` on UKMBP_CMS_SGM (FSCM credit limit) and BSID (open balance) |
